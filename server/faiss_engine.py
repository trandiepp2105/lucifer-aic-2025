...
#                 # Lấy thông tin VRAM còn trống
#                 if torch.cuda.is_available():
#                     torch.cuda.set_device(gpu_id)
#                     free_memory, total_memory = torch.cuda.mem_get_info(gpu_id)
#                     # Sử dụng 80% VRAM còn trống cho Faiss
#                     temp_memory = int(free_memory * 0.8)
#                     print(f"📊 GPU {gpu_id}: Total={total_memory/1e9:.2f}GB, Free={free_memory/1e9:.2f}GB")
#                     print(f"🎯 Allocating {temp_memory/1e9:.2f}GB for Faiss temporary memory")
#                 else:
#                     # Fallback to 8GB nếu không detect được GPU
#                     temp_memory = 8 * 1024 * 1024 * 1024
#                     print(f"⚠️ Cannot detect GPU memory, using default 8GB")
                
#                 res = faiss.StandardGpuResources()
#                 # Set temporary memory để tránh lỗi StackDeviceMemory
#                 res.setTempMemory(temp_memory)
#                 # Disable use of pre-allocated memory stack
#                 res.noTempMemory()
#                 self.gpu_resources_map[gpu_id] = res
