...
object DRES {
    ...
    /**
     * Ensures that a default admin user exists. Creates one if no admin user is found.
     */
    private fun ensureDefaultAdminUser() {
        try {
            val users = UserManager.list()
            val hasAdmin = users.any { it.role?.name == "ADMIN" }
            
            if (!hasAdmin) {
                val defaultUsername = "lucifer"
                val defaultPassword = "lucifer@"
                
                println("No admin user found. Creating default admin user...")
                val adminUser = UserManager.create(
                    username = defaultUsername,
                    password = Password.Plain(defaultPassword).hash(),
                    role = ApiRole.valueOf("ADMIN")!!
                )
                println("Default admin user created: $defaultUsername / $defaultPassword")
                println("⚠️  SECURITY WARNING: Please change the default password immediately!")
            } else {
                println("Admin user already exists.")
            }
        } catch (e: Exception) {
            println("Failed to check/create default admin user: ${e.message}")
        }
    }
}
