...
/**
 * A [Scoreboard] that keeps track of the raw (non-normalized) score per [DbTaskTemplate] as identified by it [TemplateId].
 * Unlike [MaxNormalizingScoreBoard], this scoreboard returns actual cumulative scores without normalization.
 *
 * @author Based on MaxNormalizingScoreBoard by Luca Rossetto & Ralph Gasser
 * @version 1.0.0
 */
class RawScoreBoard(override val name: String, override val run: EvaluationRun, private val teamIds: List<TeamId>, private val taskFilter: (ApiTaskTemplate) -> Boolean, private val taskGroupName: String? = null) : Scoreboard {

    /** A [ReentrantReadWriteLock] to synchronise access to this [RawScoreBoard]. */
    ...
    /** Flag indicating, that this [RawScoreBoard] is dirty and needs re-calculation. */
    @Volatile
    override var dirty: Boolean = true
        private set

    /**
     * Returns all overall [Score]s tracked by this [RawScoreBoard].
     *
     * @return List of [Score] for this [RawScoreBoard].
     */
    ...
    /**
     * Returns a summary of all current scores in this [RawScoreBoard]
     */
    override fun overview(): ScoreOverview = ScoreOverview(this.name, this.taskGroupName, this.scores())

    /**
     * Invalidates the content held by this [RawScoreBoard].
     */
    ...
    /**
     * Internal function; calculates the overall score per team without normalization.
     */
    private fun recalculate() {
        ...
        /* Update local score map with RAW SCORES (no normalization) */
        this.lock.write {
            this.scores = scoreSums  // Use raw cumulative scores directly
            this.dirty = false
        }
