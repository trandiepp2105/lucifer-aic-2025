...
import dev.dres.api.rest.types.collection.ApiCreateMediaItemRequest
...
object MediaCollectionManager {
    ...
    fun addMediaItem(mediaItemRequest: ApiCreateMediaItemRequest) {

        val collectionId = mediaItemRequest.collectionId

        this.store.transactional {
            val collection = DbMediaCollection.query(DbMediaCollection::id eq collectionId).firstOrNull()
                ?: throw IllegalArgumentException("Invalid parameters, collection with ID $collectionId does not exist.")
            if (collection.items.filter { it.name eq mediaItemRequest.name }.isNotEmpty) {
                throw IllegalArgumentException("Media item with name '${mediaItemRequest.name}' already exists in collection ${collection.name}.")
            }

            val item = DbMediaItem.new {
                this.type = mediaItemRequest.type.toDb()
                this.name = mediaItemRequest.name
                this.location = mediaItemRequest.location
                this.fps = mediaItemRequest.fps
                this.durationMs = mediaItemRequest.durationMs
            }
            collection.items.add(item)
        }
