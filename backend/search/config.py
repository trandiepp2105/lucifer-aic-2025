...
def get_dynamic_datasets():
    """
    Dynamically discover OCR datasets from CONTAINER_INDEX_DIR environment variable.
    ...
    """
    container_index_dir = os.getenv('CONTAINER_INDEX_DIR')
    
    if not container_index_dir:
        # Fallback to hardcoded datasets if env var not set
        return [
            ('/backend/index/ocr-data/viet-ocr-json-data', "viet_ocr_index"),
            ('/backend/index/ocr-data/parseq-ocr-json-data', "parseq_ocr_index")
        ]
    
    # Build ocr-data path
    ocr_data_dir = os.path.join(container_index_dir, 'ocr-data')
