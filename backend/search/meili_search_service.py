...
class MeiliSearchService(metaclass=SingletonMeta):
    ...
    def check_memory_usage(self):
        """
        Check MeiliSearch memory usage and index status
        """
        try:
            stats = self.client.get_stats()
            logger.info("=== MeiliSearch Memory Status ===")
            logger.info(f"Database size: {stats.get('databaseSize', 'Unknown')}")
            logger.info(f"Used database size: {stats.get('usedDatabaseSize', 'Unknown')}")
            ...
            for index_name in self.index_names:
                try:
                    ...
                    logger.info(f"  - Documents: {index_stats.get('numberOfDocuments', 0)}")
                    logger.info(f"  - Indexing: {index_stats.get('isIndexing', False)}")
                    logger.info(f"  - Field distribution: {index_stats.get('fieldDistribution', {})}")
                except Exception as e:
